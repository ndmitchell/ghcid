...
def require_env_setup() -> None:
    env_file = Path(".env")
    if not env_file.exists():
        print("No .env file found.", file=sys.stderr)
        print("Run `cp .env.example .env` and fill in the required variables.", file=sys.stderr)
        raise SystemExit(1)

    password = os.environ.get("WINDOWS_SSH_PASSWORD", "").strip()
    if password:
        return

    generated = gen_password()
    print("WINDOWS_SSH_PASSWORD is empty.", file=sys.stderr)
    print(f"Generated password: {generated}", file=sys.stderr)
    print("Add it to your .env and run: dotenv run -- python3 ssh-windows-gcp.py", file=sys.stderr)
    raise SystemExit(1)
